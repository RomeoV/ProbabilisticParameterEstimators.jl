...
@testset "ProbabilisticParameterEstimators.jl" begin
    ...
    @testset "predictsamples and predictdist" begin
        @testset "univariate measurements" begin
            ...
            noises = [0.001 * rand() * Normal() for _ in eachindex(xs)]
        ...
        @testset "multivariate measurements" begin
            ...
            noises = [(rand() / 100)^2 * I(2) * MvNormal(zeros(2), I) for _ in eachindex(xs)]
        ...
        @testset "correlated gaussian noise" begin
            ...
            prior = MvNormal(θtrue, 0.5^2 * I)
            n = length(xs)
            corrnoise = (1 / 100)^2 * I(n) * MvNormal(zeros(n), I(n) + 1 / 5 * hermitianpart(rand(n, n)))
        ...
        @testset "multivariate measurements correlated gaussian noise" begin
            ...
            corrnoise = let n = 2 * n
                ...
                (1 / 100)^2 * I(n) * MvNormal(zeros(n), Σ)
            end
        ...
        @testset "product noise" begin
            ...
            xs = 5 * eachcol(rand(2, 8))
            θtrue = [5.0, 10]
            prior = MvNormal(θtrue, 0.1^2 * I)
            n = length(xs)
            productnoise = [truncated(0.001 * Normal(), -0.001, Inf) for _ in 1:length(xs)]
